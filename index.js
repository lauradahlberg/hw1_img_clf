...
//adding the webcam element
const webcamElement = document.getElementById('webcam');
let net;

// creating an infinite loop which makes predictions through the webcam element
async function app() {
  ...
  while (true) {
    if (classifier.getNumClasses() > 0) {
      ...
      const activation = net.infer(webcamElement, 'conv-preds');
    ...
    // give some breathing room by waiting for the next animation nextFrame
    // to fire.
    await tf.nextFrame();
  }
}

// adding a webcam setup function
